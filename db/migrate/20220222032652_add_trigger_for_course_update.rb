class AddTriggerForCourseUpdate < ActiveRecord::Migration[7.0]
  def change
    execute <<~SQL
      ...
      BEGIN
          IF (NEW.capacity > OLD.capacity OR NEW.waitlist_capacity > OLD.waitlist_capacity) THEN
              ...
              IF (available_capacity > 0) THEN
                  SELECT count(student_id) INTO wait_listed_students FROM enrolls WHERE course_id = NEW.code and waitlist = true;
                  UPDATE enrolls SET waitlist = false WHERE course_id = NEW.code and student_id IN
                    (SELECT student_id FROM enrolls WHERE course_id = NEW.code and waitlist = true ORDER BY created_at LIMIT available_capacity);
