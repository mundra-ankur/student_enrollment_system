CREATE OR REPLACE FUNCTION process_student_enrollment() RETURNS TRIGGER AS
$student_enrollment$
DECLARE
    course_code              varchar;
...
BEGIN
    IF (TG_OP = 'DELETE') THEN
        ...
        SELECT capacity, waitlist_capacity, status, enrolled, wait_listed
        INTO
        ...
        ELSE
            ...
            ELSIF (course_status = 'WAITLIST') THEN
                ...
                ELSE
                    course_waitlist = course_waitlist - 1;
                    UPDATE public.enrolls
                    SET waitlist = false
                    WHERE student_id =
                          (SELECT student_id
                           FROM public.enrolls
                           WHERE course_id = course_code AND waitlist = true
                           ORDER BY created_at
                           LIMIT 1);
                END IF;
            ELSE
...
CREATE TRIGGER student_enrollment_del
    AFTER DELETE
    ON public.enrolls
    FOR EACH ROW
EXECUTE FUNCTION process_student_enrollment();

CREATE TRIGGER student_enrollment_ins
    BEFORE INSERT
    ON public.enrolls
    FOR EACH ROW
EXECUTE FUNCTION process_student_enrollment();